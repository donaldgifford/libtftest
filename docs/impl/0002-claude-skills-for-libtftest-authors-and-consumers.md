...
- [x] **`libtftest-reviewer` (agent in this repo)**
  - [x] `.claude/agents/libtftest-reviewer.md` with frontmatter declaring
        the agent's name, description, and `disallowedTools: Write, Edit`
  - [x] System prompt: enforces only libtftest-specific rules (`tb` not
        `t`, `Seed*` cleanup pairing, `<Service>` namespace var,
        `PortEndpoint` vs `Endpoint`, `BuildOptions` vs `BuildPlanOptions`,
        `aws.Config` by value, RequirePro gating, no `:latest` LocalStack
        image, nolint discipline). Defers Go style / architecture review
        to `go-development:go-style` and `go-development:go-architect`
        (Resolved Decision #6)
  - [x] Output contract: end with a JSON block of structured findings:
        `{ severity, file, line, rule, message }[]`. Documented rule
        identifiers (naming.tb-not-t, cleanup.no-pair, edition.no-requirepro,
        tc.endpoint-not-portendpoint, etc.) so golden-file diffs are stable
  - [~] (deferred) Golden-file regression on PR #6 / v0.1.0 diff — runs
        in Phase 6 once the agent test harness exists
- [x] **`tftest-reviewer` (agent in `libtftest` plugin)**
  - [x] `plugins/libtftest/agents/tftest-reviewer.md` with frontmatter
  - [x] System prompt: review checklist covering `tc.Prefix()` usage,
        cleanup ordering (fixture-before-Apply), edition gating,
        hardcoded credentials, output coverage, build tags, TestMain
        duplication, tc.SetVar/Output ordering
  - [x] Same JSON-findings output contract as `libtftest-reviewer`
        (Resolved Decision #6) with rule identifiers documented
  - [x] Flags out-of-range libtftest version as first finding when
        applicable; recommends `/tftest:upgrade`
